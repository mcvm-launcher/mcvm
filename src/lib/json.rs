...
pub fn access_i64(obj: &JsonObject, key: &str) -> Result<i64, JsonError> {
	match obj.get(key) {
		Some(val) => match val.as_i64() {
			Some(val) => Ok(val),
			None => Err(JsonError::Type(key.to_string(), JsonType::Int))
		},
...
pub fn access_f64(obj: &JsonObject, key: &str) -> Result<f64, JsonError> {
	match obj.get(key) {
		Some(val) => match val.as_f64() {
			Some(val) => Ok(val),
			None => Err(JsonError::Type(key.to_string(), JsonType::Float))
		},
		None => Err(JsonError::Key(key.to_string()))
	}
}
...
pub fn access_bool(obj: &JsonObject, key: &str) -> Result<bool, JsonError> {
	match obj.get(key) {
		Some(val) => match val.as_bool() {
			Some(val) => Ok(val),
			None => Err(JsonError::Type(key.to_string(), JsonType::Bool))
		},
		None => Err(JsonError::Key(key.to_string()))
	}
}
...
pub fn access_str<'a>(obj: &'a JsonObject, key: &str) -> Result<&'a str, JsonError> {
	match obj.get(key) {
		Some(val) => match val.as_str() {
			Some(val) => Ok(val),
			None => Err(JsonError::Type(key.to_string(), JsonType::Str))
		},
		None => Err(JsonError::Key(key.to_string()))
	}
}
...
pub fn access_array<'a>(obj: &'a JsonObject, key: &str) -> Result<&'a Vec<Value>, JsonError> {
	match obj.get(key) {
		Some(val) => match val.as_array() {
			Some(val) => Ok(val),
			None => Err(JsonError::Type(key.to_string(), JsonType::Array))
		},
		None => Err(JsonError::Key(key.to_string()))
	}
}
...
pub fn access_object<'a>(obj: &'a JsonObject, key: &str) -> Result<&'a JsonObject, JsonError> {
	match obj.get(key) {
		Some(val) => match val.as_object() {
			Some(val) => Ok(val),
			None => Err(JsonError::Type(key.to_string(), JsonType::Object))
		},
		None => Err(JsonError::Key(key.to_string()))
	}
}
...
pub fn ensure_type<T>(value: Option<T>, key: &str, typ: JsonType) -> Result<T, JsonError> {
	match value {
		Some(val) => Ok(val),
		None => Err(JsonError::Type(key.to_string(), typ))
	}
}
